...
genome1_path = "/Users/jdurazo/Desktop/Projects/Sequencing/CAassemblies/Allfastas/example2/contigs.fasta"
genome2_path = "/Users/jdurazo/Desktop/Projects/Sequencing/CAassemblies/Allfastas/example2/neoreferencegenome.fasta"
...
delta_file = f"contigs_neoreferencegenome.delta"
coords_file = f"contigs_neoreferencegenome.coords"
...
# Convert contigs dictionary to list of tuples (name, sequence)
genome1_contigs = [(name, seq) for name, seq in genome1.contigs.items()]
genome2_contigs = [(name, seq) for name, seq in genome2.contigs.items()]

# Sort contigs by size
genome1_contigs.sort(key=lambda x: len(x[1]), reverse=True)
genome2_contigs.sort(key=lambda x: len(x[1]), reverse=True)

# Update contigs attribute with sorted list
genome1.contigs = dict(genome1_contigs)
genome2.contigs = dict(genome2_contigs)
